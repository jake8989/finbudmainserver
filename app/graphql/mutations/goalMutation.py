...
from app.graphql.schemas.GoalSchema import (
    GoalType,
    GoalInput,
    GoalReponseType,
    DeleteGoalInputType,
    EditGoalInputType,
    AllUserGoalsResponseType,
)
...
class GoalMutation:
    ...
    async def addNewGoal(goal: GoalInput) -> GoalReponseType:
        try:
            ...
            if len(allUserGoals) >= 6:
                return GoalReponseType(
                    success=False, message="you cannot create more than 6 goals"
                )

            existing_goal = next(
                (g for g in allUserGoals if g["goalCategory"] == goal.goalCategory),
                None,
            )
            if existing_goal:
                return GoalReponseType(
                    success=False, message="A goal with same category exists!"
                )
            ...
            await database.db["users"].find_one_and_update(
                {"username": goal.username}, {"$set": {"goals": allUserGoals}}
            )
            await database.db["users"].find_one_and_update(
                {"username": goal.username},
                {"$set": {"settedGoals": exist_user["settedGoals"] + 1}},
            )
            # graphql expects the goal to be a object not a dictionary so converting back to object
            newGoal = GoalType(**newGoal)
            return GoalReponseType(
                success=True, message="goal added successfully!", goal=newGoal
            )
    ...
    async def deleteGoal(goal: DeleteGoalInputType) -> GoalReponseType:
        try:
            ...
            goalsAfterDeletion = [
                goal for goal in allUsersGoals if goal["goalId"] != goalIdToBeDeleted
            ]
            ...
            await database.db["users"].find_one_and_update(
                ...
                {
                    "$set": {
                        "goals": goalsAfterDeletion,
                        "settedGoals": exist_user["settedGoals"] - 1,
                    }
                },
            )
    ...
    async def editGoal(goal: EditGoalInputType) -> GoalReponseType:
        try:
            ...
            result = await database.db["users"].update_one(
                {"username": goal.username, "goals.goalId": goal.goalId},
                {"$set": updationRequired},
            )
            # print(result)
            user = await database.db["users"].find_one({"username": goal.username})
            updateGoal = next(
                (g for g in user["goals"] if g["goalId"] == goal.goalId), None
            )
            ...
            return GoalReponseType(
                success=True,
                message=f"Goal with ID {goal.goalId} updated successfully.",
                goal=GoalType(**updateGoal),
            )
