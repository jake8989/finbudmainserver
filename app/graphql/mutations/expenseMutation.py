...
class ExpenseMutation:
        ...
        try:
            ...
            if not exist_user:
                return ExpenseResponseType(
                    success=False,
                    message="This action cannot be performed please login again to continue",
                )
            ...
            exist_goal_with_same_category = next(
                (g for g in allUserGoals if g["goalCategory"] == expense.category), None
            )
            ...
            if exist_goal_with_same_category:
                if exist_goal_with_same_category["goalType"] == "Spending Goal":
                    ...
                    await database.db["users"].update_one(
                        {
                            "username": expense.username,
                            "goals.goalCategory": exist_goal_with_same_category[
                                "goalCategory"
                            ],
                        },
                        {"$set": {"goals.$.goalAmount": newGoalAmount}},
                    )

            await database.db["expenses"].insert_one(
                new_expense.model_dump(by_alias=True)
            )
    ...
    @staticmethod
    async def createNewExpenseCategory(
        self, expenseCategory: ExpenseCategoryInput
    ) -> ExpenseCategoryResponse:
        try:
            existUser = await database.db["users"].find_one(
            ...
            if newCategory in allExpenseCategories:
                return ExpenseCategoryResponse(
                    success=False, message="Expense Category Already Exists!"
                )
            ...
            await database.db["users"].update_one(
                {"username": expenseCategory.username},
                {"$set": {"expenseCategories": allExpenseCategories}},
            )
            return ExpenseCategoryResponse(
                success=True, message="Expense Category Added Succesfully"
            )
    ...
    @staticmethod
    async def deleteExistExpenseCategory(
        self, expenseCategory: ExpenseCategoryInput
    ) -> ExpenseCategoryResponse:
        try:
            existUser = await database.db["users"].find_one(
            ...
            if not expenseCategory.expenseCategory:
                return ExpenseCategoryResponse(
            ...
            if expenseCategoryToBeDeleted in allExpenseCategories:
                updated_categories = [
                    cat
                    for cat in allExpenseCategories
                    if cat != expenseCategoryToBeDeleted
                ]
                await database.db["users"].update_one(
                    {"username": expenseCategory.username},
                    {"$set": {"expenseCategories": updated_categories}},
                )
                return ExpenseCategoryResponse(
                    success=True, message="Expense Category Removed!"
                )
