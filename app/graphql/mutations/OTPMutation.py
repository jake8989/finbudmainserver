    ...
    async def generateAndSendOTP(otp: OTPinput) -> OTPSendResponseType:
        try:
            ...
            async with grpc.aio.insecure_channel(server_port) as channel:
                ...
                exits_otp_ratelimiter = await database.db["otp_rate_limiters"].find_one(
                    {"email": otp.email}
                )
                if exits_otp_ratelimiter:
                    if exits_otp_ratelimiter["times"] > 4:
                        return OTPSendResponseType(
                            success=False,
                            message="OTP generation limit exceeds! please use other email!",
                        )

                if response.success:
                    print("Success!")
                    if not exits_otp_ratelimiter:
                        await database.db["otp_rate_limiters"].insert_one(
                            new_otp_ratelimit.model_dump(by_alias=True)
                        )

                    if exits_otp_ratelimiter:
                        await database.db["otp_rate_limiters"].update_one(
                            {"email": otp.email},
                            {"$set": {"times": exits_otp_ratelimiter["times"] + 1}},
                        )
            ...
            return OTPSendResponseType(
                success=False, message=f"gRPC Error: {e.details()}"
            )
    ...
    async def verifyOTP(otp: VerifyOTPType) -> OTPSendResponseType:
        try:
            ...
            async with grpc.aio.insecure_channel(server_port) as channel:
                ...
                if response.success:
                    print("OK!")
                    temp_user = await database.db["temp_users"].find_one(
                        {"email": otp.email}
                    )
