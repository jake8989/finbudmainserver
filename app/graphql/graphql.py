...
from app.graphql.mutations.expenseMutation import (
    ExpenseMutation,
...
from app.graphql.schemas.ExpenseSchema import (
    ExpenseResponseType,
...
@strawberry.type
class Mutation:
    registerUser: UserMutationResponse = strawberry.mutation(
        resolver=UserMutations.registerUser
    )
    loginUser: UserMutationResponse = strawberry.mutation(
        resolver=UserMutations.loginUser
    )
    createExpense: ExpenseResponseType = strawberry.mutation(
        resolver=ExpenseMutation.createExpense
    )
    createExpenseCategory: ExpenseCategoryResponse = strawberry.mutation(
        resolver=ExpenseMutation.createNewExpenseCategory
    )
    deleteExistExpenseCategory: ExpenseCategoryResponse = strawberry.mutation(
        resolver=ExpenseMutation.deleteExistExpenseCategory
    )
    ...
    addIncome: AddIncomeResponseType = strawberry.mutation(
        resolver=IncomeMutation.createIncome
    )
    newFeedBack: FeedBackResponseType = strawberry.mutation(
        resolver=FeedBackMutation.newFeedBack
    )
    generateAndSendOTP: OTPSendResponseType = strawberry.mutation(
...
class Query:
    ...
    getAllExpenseCategories: ExpenseGetCategoriesResponseType = strawberry.mutation(
        resolver=ExpenseQuery.getAllExpenseCategories
    )
    getAllUserGoals: AllUserGoalsResponseType = strawberry.mutation(
        resolver=GoalQuery.getAllUserGoals
    )
    getAccumulatedData: accumulatedDataResponseType = strawberry.mutation(
        resolver=ChartQuery.getAccumulatedData
    )
